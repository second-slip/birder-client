...
describe('LoginComponent', () => {
  ...
  const setup = async (
    ...
    fakeTokenService = jasmine.createSpyObj<TokenService>('TokenService', {
      addToken: undefined,
      getToken: '',
  ...
  describe('form completion and submission', async () => {
    ...
    it('should show required validation messages when username input is touched but empty', async () => {
      ...
      const formField = await loader.getHarness(
        MatFormFieldHarness.with({ selector: '#username' })
      );
      ...
      expect(
        compiled.querySelector('.validation-required-username')?.textContent
      ).toBeDefined();
      expect(
        compiled.querySelector('.validation-required-username')?.textContent
    ...
    it('should show Username email format validation messages when input invalid', async () => {
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(
        compiled.querySelector('.validation-email-username')?.textContent
      ).toBeDefined();
      expect(
        compiled.querySelector('.validation-email-username')?.textContent
    ...
    it('the Login button should be disabled unless all inputs are valid', async () => {
      ...
      const input1 = await loader.getHarness(
        MatInputHarness.with({ selector: '#username' })
      );
  ...
  describe('handles service NEXT notification', () => {
    it('should handle the auth token and update the auth status', async () => {
      ...
      expect(fakeTokenService.addToken).toHaveBeenCalledOnceWith(
        authSuccessResult.authenticationToken
      );
    ...
    it('hide the successfully completed form and show the success alert banner', async () => {
      ...
      expect(
        isFormHidden.querySelector('[data-testid="loginForm"]')?.textContent
      ).toBeUndefined();
    });
  });
  ...
  describe('handles service ERROR notification', () => {
    ...
    it('shows error alert banner', async () => {
      ...
      expectText(
        fixture,
        'error',
        'Sign in failed There was an error logging in. Make sure you have typed the correct email and password. If you have forgotten your password, you can reset it. '
      );
    });
  });
});
