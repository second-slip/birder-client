...
describe('ObservationUpdateComponent', () => {
  ...
  let componentRef: ComponentRef<ObservationUpdateComponent>;
  ...
  const setup = async (
    ...
    componentRef.setInput('id', fakeRouteArgument);
  ...
  describe('when component is created', () => {
    ...
    it('should get the id from the route and call data fetch', async () => {
      ...
      expect(
        fakeObservationCrudService.getObservation
      ).toHaveBeenCalledOnceWith(expectedRouteArgument);
    });
  ...
  describe('when record cannot be fetched', () => {
    it('should show error section', async () => {
      ...
      expect(
        compiled.querySelector('[data-testid="loading-error-section"]')
      ).toBeTruthy();
    ...
    it('should retry on button click', async () => {
      ...
      expect(fakeObservationCrudService.getObservation).toHaveBeenCalledTimes(
        2
      );
    });
  });
  ...
  describe('when observation is successfully loaded', () => {
    ...
    it('should load quantity input with correct setup', async () => {
      ...
      expect(await input.getValue()).toBe(
        updateObservationModel.quantity.toString()
      );
    });
    ...
    it('should show valid form menu', async () => {
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(
        compiled.querySelector('[data-testid="valid-form-menu"]')
      ).toBeTruthy();
      expect(
        compiled.querySelector('[data-testid="valid-form-menu"]')?.textContent
      ).toContain('The form is complete. Choose "Update" to save the changes.');
    });
  });
  ...
  describe('checks the authorisation status', () => {
    it('should show "not authorised" message if not record owner', async () => {
      ...
      const notAuthorisedUser = <IAuthUser>{
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(
        compiled.querySelector('[data-testid="not-authorised"]')
      ).toBeTruthy();
      expect(
        compiled.querySelector('[data-testid="not-authorised"]')?.textContent
      ).toContain('Only the record owner is allowed to update their record');
    });
  ...
  describe('filling out the form', () => {
    ...
    it('should show invalid form menu when form is not valid', async () => {
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(
        compiled.querySelector('[data-testid="invalid-form-menu"]')
      ).toBeTruthy();
      expect(
  ...
  describe('to submit the form', () => {
    it('should submit the valid form and call the service', async () => {
      ...
      expect(
        fakeObservationCrudService.updateObservation
      ).toHaveBeenCalledTimes(1);
      expect(
        fakeAnnounceChangesService.announceObservationsChanged
      ).toHaveBeenCalledTimes(1);
      expect(router.navigate).toHaveBeenCalledWith([
        '/observation/detail/' + expectedRouteArgument,
      ]);
    });
    ...
    it('show error message on service error', async () => {
      ...
      await setup(
        {
          getObservation: of(updateObservationModel),
          updateObservation: throwError(
            () => new Error('location update error')
          ),
        },
        {
          getAuthUser: of(userModel),
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(
        compiled.querySelector('[data-testid="record-update-error"]')
      ).toBeTruthy();
      expect(
        compiled.querySelector('[data-testid="record-update-error"]')
          ?.textContent
