...
describe('UserProfileComponent', () => {
  ...
  const setup = async (
    fakePropertyValues?: jasmine.SpyObjPropertyNames<UserProfileService>
  ) => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(UserProfileComponent, {
        ...
        add: {
          ...
          providers: [{ provide: UserProfileService, useValue: fakeService }],
        },
      })
      .compileComponents();
  ...
  describe('when the response is successful', () => {
    it('shows the user profile', async () => {
      await setup({
      ...
      expect(
        compiled.querySelector('[data-testid="user-profile"]')?.textContent
      ).toBeDefined();

      expectText(fixture, 'h1-user-name', username);
    });

    it('shows the network summary', async () => {
      await setup({
        getUserProfile: of(userProfileModel),
      });
      ...
      const compiled = fixture.nativeElement as HTMLElement;
      expect(
        compiled.querySelector('[data-testid="network-summary"]')?.textContent
      ).toBeDefined();
    ...
    describe('when viewing OWN PROFILE', () => {
      it('shows the observations count summary from the ObservationCountService', async () => {
        await setup({
          getUserProfile: of(userProfileModel),
        });
        ...
        const compiled = fixture.nativeElement as HTMLElement;
        expect(
          compiled.querySelector('[data-testid="obs-summary-own"]')?.textContent
        ).toBeDefined();
    ...
    describe('when viewing OTHER PROFILE', () => {
      it('shows the observations count summary from userProfileModel', async () => {
        ...
        const compiled = fixture.nativeElement as HTMLElement;
        expect(
          compiled.querySelector('[data-testid="obs-summary-other"]')
            ?.textContent
        ).toBeDefined();
